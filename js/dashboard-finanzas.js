...
(function (global) {
    ...
    function getAcceptedDebtRows() {
        ...
        return cots
            ...
            .filter(c => Number(c.saldoPendiente) > 0.01)
    ...
    function getPaymentTransactions() {
        ...
        cots.forEach(c => {
            ...
            // Compatibilidad con registros antiguos donde el cobro vive en el principal.
    ...
    function buildFinancialCenter() {
        ...
        const kpi = (icon, label, value, note) => `
            <div style="padding:14px;border:1px solid var(--border);border-radius:var(--radius);background:var(--surface);min-width:0;">
                <div style="font-size:11px;color:var(--text-soft);text-transform:uppercase;letter-spacing:.04em;">${icon} ${label}</div>
                <div style="font-size:20px;font-weight:650;color:var(--primary);margin-top:5px;">${value}</div>
                <div style="font-size:11px;color:var(--text-soft);margin-top:3px;">${note}</div>
            </div>`;

        const carteraHtml = cartera.length
            ? cartera.slice(0, 8).map(c => `
                <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);align-items:center;">
                    <div style="min-width:0;">
                        <div style="font-size:13px;font-weight:550;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(c.titulo || 'Sin título')}</div>
                        <div style="font-size:11px;color:var(--text-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(c.cliente || '')} · ${Number(c.porcentaje || 0).toFixed(0)}% cobrado</div>
                    </div>
                    <div style="text-align:right;font-size:13px;font-weight:600;">${bs(c.saldoPendiente)}</div>
                </div>`).join('')
            : `<div style="padding:16px 0;color:var(--text-soft);font-size:13px;">No hay cartera pendiente.</div>`;

        return `
            <div class="panel" id="centro-financiero" style="margin-top:18px;">
                <div class="panel-h">
                    <div><h3>💰 Centro Financiero</h3><span>Resumen de cartera y comportamiento de cobros</span></div>
                    <button class="btn btn-sm btn-ghost" onclick="S.view='pagos';render();">Gestionar pagos</button>
                </div>
                <div class="panel-body">
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:18px;">
                        ${kpi('💵', 'Cobrado acumulado', bs(summary.totalCobrado), `${transactions.length} registros de cobro`)}
                        ${kpi('📥', 'Por cobrar', bs(summary.totalPorCobrar), `${cartera.length} deudas pendientes`)}
                        ${kpi('⏳', 'Deudas pendientes', summary.deudasPendientes, 'Sin pagos registrados')}
                        ${kpi('🔄', 'Pagos parciales', summary.deudasParciales, 'Cotizaciones con saldo')}
                        ${kpi('✅', 'Pagos completados', summary.deudasPagadas, 'Sin saldo pendiente')}
                    </div>

                    <div style="display:grid;grid-template-columns:minmax(0,1.15fr) minmax(280px,.85fr);gap:18px;align-items:start;">
                        <div>
                            <div style="font-size:13px;font-weight:600;margin-bottom:6px;">📌 Cartera pendiente</div>
                            ${carteraHtml}
                            ${cartera.length > 8 ? `<div style="font-size:11px;color:var(--text-soft);padding-top:8px;">Mostrando las 8 deudas con mayor saldo.</div>` : ''}
                        </div>
                        <div>
                            <div style="font-size:13px;font-weight:600;margin-bottom:10px;">📈 Cobros por mes</div>
                            <div style="display:flex;align-items:flex-end;gap:8px;height:130px;padding:5px 2px 0;">
                                ${months.map(m => `
                                    <div style="flex:1;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;gap:5px;min-width:0;">
                                        <div title="${m.label}: ${bs(m.total)}" style="width:100%;max-width:42px;height:${Math.max(3, (m.total / maxMonth) * 100)}px;border-radius:4px 4px 0 0;background:var(--accent);"></div>
                                        <span style="font-size:10px;color:var(--text-soft);text-transform:capitalize;">${m.label}</span>
                                    </div>`).join('')}
                            </div>
                        </div>
                    </div>

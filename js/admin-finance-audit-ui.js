...
// Solo lectura. No modifica S ni Firebase.
// ============================================================
(function (global) {
    'use strict';
    ...
    function getFacturadoRows() {
        return (Array.isArray(S.pagos) ? S.pagos : []).filter(p => Number(p?.monto || 0) > 0);
    }

    function getCobradoRows() {
        return (Array.isArray(S.pagos) ? S.pagos : []).filter(p => Number(p?.montoPagado || 0) > 0 || Number(p?.monto || 0) > 0);
    }
    ...
    function showAudit(title, rows, type) {
        ...
        const total = rows.reduce((s, p) => {
            if (type === 'deuda') return s + Number(p.saldoPendiente || 0);
            if (type === 'cobrado') return s + Number(p.montoPagado || 0);
        ...
        <div class="modal" style="max-width:1000px;width:94vw;">
            <div class="modal-h">
                <h3>🔎 ${safe(title)}</h3>
                <button class="close" id="audit-close">&times;</button>
            </div>
            <div class="modal-body">
                <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:14px;padding:10px 12px;background:var(--gantt-bg);border-radius:var(--radius);">
                    <strong>${rows.length} registros considerados</strong>
                    <strong style="color:var(--primary);">Total: ${money(total)}</strong>
                </div>
                ${rows.length ? `
                <div class="table-wrap"><table>
                    <thead><tr>
                        <th>Fecha</th><th>Descripción</th><th>Cliente</th><th>Proyecto</th>
                        <th class="tright">Monto</th><th class="tright">Pagado</th><th class="tright">Saldo</th><th>Origen</th>
                    </tr></thead>
                    <tbody>
                    ${rows.map(p => {
                        ...
                        return `<tr>
                            <td class="tnum" style="white-space:nowrap;">${typeof fmtDate==='function'?fmtDate(p.fecha||p.fechaCreacion||''):'—'}</td>
                            <td>${safe(p.titulo || p.descripcion || p.concepto || p.nombre || '—')}</td>
                            <td>${safe(p.cliente || p.entidad || '—')}</td>
                            <td>${safe(proyecto || '—')}</td>
                            <td class="tright tnum">${money(monto)}</td>
                            <td class="tright tnum" style="color:var(--success);">${money(pagado)}</td>
                            <td class="tright tnum" style="color:${saldo>0?'var(--danger)':'var(--success)'};">${money(saldo)}</td>
                            <td>${safe(p.origen || (p.cotizacionId ? 'Cotización' : 'Administrativo'))}</td>
                        </tr>`;
                    }).join('')}
                    </tbody>
                </table></div>` : '<div class="empty">No hay registros considerados.</div>'}
            </div>
    ...
    const observer = new MutationObserver(() => {
        if (S.view === 'administrativo') setTimeout(enhance, 0);
    });

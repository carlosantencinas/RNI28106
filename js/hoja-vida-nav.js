...
function addDesktop(nav){
 if(!nav||nav.classList.contains('ios-nav'))return;
 cleanDesktop(nav);
 if(nav.querySelector('[data-hv-entry]'))return;
 const group=document.createElement('div');
 group.className='nav-group';
 group.dataset.hvGroup='1';
 group.innerHTML='<div class="nav-group-title">Trayectoria</div><button class="nav-btn" data-hv-entry title="Hoja de Vida"><span class="nav-icon">📄</span><span class="nav-label">Hoja de Vida</span></button>';
 nav.appendChild(group);
 group.querySelector('[data-hv-entry]').onclick=function(e){e.preventDefault();e.stopPropagation();openHV();};
}
function addIOS(){
 const select=document.getElementById('ios-nav-select');
 if(!select)return;
 if(!select.querySelector('option[value="hoja-vida"]')){
...
function addOverlay(){
 ...
 if(!menu||menu.querySelector('[data-hv-entry]'))return;
 const section=document.createElement('div');
 section.style.cssText='padding:12px 16px;border-bottom:1px solid #EEE;';
 section.innerHTML='<div style="font-size:10px;font-weight:700;color:#999;text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px;">Trayectoria</div><button class="nav-btn-overlay" data-hv-entry style="display:block;width:100%;text-align:left;padding:10px 12px;border:none;background:none;font-size:15px;color:#111;cursor:pointer;">📄 Hoja de Vida</button>';
 section.querySelector('[data-hv-entry]').onclick=function(e){e.preventDefault();e.stopPropagation();overlay.remove();openHV();};
...
function patchOverlay(){
 ...
 window.toggleMobileMenuOverlay=function(){original();setTimeout(addOverlay,20);};

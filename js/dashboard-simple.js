...
function viewDashboard() {
    ...
    function kpi(icon,label,value,sub,cls='') {
        return `<div class="dash-kpi ${cls}"><div class="dash-kpi-top"><span class="dash-icon">${icon}</span><span class="dash-kpi-label">${label}</span></div><div class="dash-kpi-value">${value}</div><div class="dash-kpi-sub">${sub}</div></div>`;
    }
    ...
    <div class="dash-kpi-grid">
        ${kpi('↗','Por cobrar',money(totalPorCobrar),`${pendientes.length} pendientes`,'danger')}
        ${kpi('✓','Cobrado',money(totalCobrado),`${pagos.length} registros`,'success')}
        ${kpi('◆','Aceptado',money(totalAceptado),`${aceptadas.length} cotizaciones`,'primary')}
        ${kpi('◇','Cotizado',money(totalCotizado),`${cots.length} propuestas`,'neutral')}
        ${kpi('⌁','Experiencia',`${expYears.toFixed(1)} años`,`${exp.length} proyectos`,'neutral')}
        ${kpi('★','Licitaciones',lic.length,`${adjudicadas} adjudicadas`,'accent')}
    </div>
...
(function(){
    ...
    @media(max-width:420px){.dash-kpi{padding:12px}.dash-kpi-value{font-size:17px}.dash-module-grid{grid-template-columns:1fr}}
